...
/*新增排名查询*/
$pzcount = pdo_fetch("SELECT count(id) as pzcount FROM " . tablename($this->tablevoteuser) . " WHERE rid = :rid AND votenum >= :votenum", array(':rid' => $rid,':votenum' => $voteuser['votenum']));
$upusercount = pdo_fetch("SELECT votenum as upusercount FROM " . tablename($this->tablevoteuser) . " WHERE rid = :rid AND votenum > :votenum ORDER BY votenum ASC", array(':rid' => $rid,':votenum' => $voteuser['votenum']));
$pcha = $upusercount['upusercount'] - $voteuser['votenum'];
if($pcha < 0){
	$pcha = 0;
} 
