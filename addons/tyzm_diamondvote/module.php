...
class tyzm_diamondvoteModule extends WeModule
{
	...
	public function fieldsFormDisplay($rid = 0)
	{
		...
		if (empty($rid)) {
			...
			if ($issetmeal && empty($_W['isfounder'])) {
				...
				if (empty($meal)) {
					$setmealstatus = 1;
					$setmealmsg = '未开通套餐，请联系管理员开通套餐，QQ：505097558';
				}
				if ($meal['status']) {
					$setmealstatus = 1;
					$setmealmsg = '套餐已禁用，请联系管理员开通，QQ：505097558';
				}
				if ($meal['starttime'] > time()) {
					$setmealstatus = 1;
					$setmealmsg = '套餐未生效，请联系管理员开通，QQ：505097558';
				}
				if ($meal['endtime'] < time()) {
					$setmealstatus = 1;
					$setmealmsg = '套餐已过期，请联系管理员开通，QQ：505097558';
				}
	...
	public function fieldsFormSubmit($rid)
	{
		...
		if (!($k < count($_POST['adtitle']))) {
			...
			if (!($k < count($_POST['gifttitle']))) {
				$style = @iserializer(array('template' => $_POST['template'], 'setstyle' => ''));
				$giftdata = @iserializer($giftdata);
				$k = 0;
				QkGVM:
				if (!($k < count($_POST['infoname']))) {
					$applydata = @iserializer($applydata);
					$insert = array('rid' => $rid, 'uniacid' => $_W['uniacid'], 'title' => $_GPC['title'], 'thumb' => $_GPC['thumb'], 'description' => $_GPC['description'], 'starttime' => strtotime($_GPC['time']['start']), 'endtime' => strtotime($_GPC['time']['end']), 'showstarttime' => strtotime($_GPC['showtime']['start']), 'showendtime' => strtotime($_GPC['showtime']['end']), 'apstarttime' => strtotime($_GPC['aptime']['start']), 'apendtime' => strtotime($_GPC['aptime']['end']), 'votestarttime' => strtotime($_GPC['votetime']['start']), 'voteendtime' => strtotime($_GPC['votetime']['end']), 'topimg' => $_GPC['topimg'], 'bgcolor' => $_GPC['bgcolor'], 'style' => $style, 'eventrule' => htmlspecialchars_decode($_GPC['eventrule']), 'prizemsg' => htmlspecialchars_decode($_GPC['prizemsg']), 'config' => $config, 'addata' => $addata, 'giftdata' => $giftdata, 'bill_data' => htmlspecialchars_decode($_GPC['bill_data']), 'applydata' => $applydata, 'area' => $_GPC['area'], 'sharetitle' => $_GPC['sharetitle'], 'shareimg' => $_GPC['shareimg'], 'sharedesc' => $_GPC['sharedesc'], 'status' => $_GPC['rstatus'], 'createtime' => time(), 'master' => $_GPC['master']);
					if (empty($id)) {
						$issetmeal = pdo_tableexists('tyzm_diamondvote_setmeal');
						if ($issetmeal && empty($_W['isfounder'])) {
							$setmealstatus = 0;
							...
							if ($totalrid >= $meal['count']) {
								$setmealstatus = 1;
								if ($meal['count']) {
									$setmealmsg = $meal['count'] . '个活动套餐已用完，请联系管理员升级套餐，QQ：505097558';
								}
							}
							if ($setmealstatus) {
								message($setmealmsg);
	...
	public function settingsDisplay($settings)
	{
		...
		if (checksubmit()) {
			...
			$dat = array('certkey' => $_GPC['certkey'], 'tencent_lbs_api_key' => $_GPC['tencent_lbs_api_key'], 'isopenweixin' => $_GPC['isopenweixin'], 'key' => $_GPC['key'], 'mchid' => $_GPC['mchid'], 'apikey' => $_GPC['apikey'], 'remote' => $remote, 'giftref' => trim($_GPC['giftref']));
	...
	public function tplmobile()
	{
		...
		if (!is_dir($path)) {
			...
			niC6p:
			if (!(false !== ($templatepath = readdir($handle)))) {
				goto mXORI;
			}
