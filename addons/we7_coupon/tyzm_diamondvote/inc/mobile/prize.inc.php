...
is_weixin();
$rid=intval($_GPC['rid']);
$reply = pdo_fetch("SELECT  rid,title,prizemsg,eventrule,sharetitle,shareimg,sharedesc,config,style,addata,endtime,apstarttime,apendtime,status  FROM " . tablename($this->tablereply) . " WHERE rid = :rid ", array(':rid' => $rid));
...
$_W['page']['sitename']=$reply['sharetitle'];
