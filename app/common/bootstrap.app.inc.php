...
if($_GPC['from']=='wxapp' || $_GPC['a']=='wxapp'){
	$setting = uni_setting($_W['uniacid'], array('payment'));
	if ($setting['payment']['wxapp']['wxappkey']) {
		$_W['account']['key'] = $setting['payment']['wxapp']['wxappkey'];
		$_W['account']['secret'] = $setting['payment']['wxapp']['wxappsecret'];
		if($_GPC['openid']){
			$_SESSION['openid'] = $_W['openid'] = $_GPC['openid'];
		}
		$_W['account']['type'] = ACCOUNT_TYPE_APP_NORMAL;
	}
	$wxappinfo = pdo_fetch("SELECT * FROM " . tablename('account_wxapp') . " WHERE uniacid = :uniacid ", array(':uniacid' => $_W['uniacid']));
	if($wxappinfo['a_uniacid']){
		$_W['wx_uniacid']  = $_W['uniacid'];
		$_W['uniacid']  = $wxappinfo['a_uniacid'];
		$_W['acid'] = $wxappinfo['acid'];
		$_W['wxappkey'] = $wxappinfo['key'];
		$_W['wxappsecret'] = $wxappinfo['secret'];
...
//cosen
if (!empty($_W['account']['oauth']) && $_W['account']['oauth']['level'] == '4' &&  empty($_W['isajax']) && $_GPC['from']!='wxapp' && $_GPC['a']!='wxapp') {
