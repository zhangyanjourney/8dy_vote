...
if ($do == 'openid') {
	...
	if (!empty($oauth) && !is_error($oauth)) {
		...
		$account_api->result(0, '', array('sessionid' => $_W['session_id'], 'session_key' => $oauth['session_key'], 'openid' => $oauth['openid']));
	} else {
	...
	if($userinfo['openId']){
		...
		$account_api->result(1, 'openid获取失败！');
	}
	$fans_update = array(
