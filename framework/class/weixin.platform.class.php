...
class WeiXinPlatform extends WeiXinAccount {
	...
	//以下为小程序一键发布代码
	public function build($uniacid,$version=''){
		...
		$template_id= trim($module['template_id']);
		if(empty($module['template_id']) && $module['template_id'] !='0'){
			itoast('上传小程序代码出错：模块小程序ID为空','', 'error');
		}
		...
		$ext=array(
			...
			'version'=>$version,
			'siteroot' => $siteroot.'app/index.php',
			'attachurl' => $siteroot.'attachment/',
			'multiid'=>'0',
			'design_method'=>'3'
		);
		$post=array(
		...
		if($result['errcode']){
			itoast('上传小程序代码出错：'.$result['errcode'].$result['errmsg'], '', 'error');
		}
		//读取可用分类
		...
		$result=json_decode($result,true);
		if($result['errcode']){
			itoast('提交审核小程序代码出错：'.$result['errcode'].$result['errmsg'], '', 'error');
		}
		pdo_update('account_wxapp',array('status'=>1,'fail_reason'=>$result['auditid']),array('uniacid'=>$uniacid));
		//提交成功，等待审核结果
		return true;
	}
	public function check($uniacid){
		...
		$wxapp=pdo_get('account_wxapp',array('uniacid'=>$uniacid));
		if($wxapp['status']==2){
			itoast('审核成功！请点击一键发布按钮发布！', '', 'success');
		}elseif($wxapp['status']==3){
			itoast('审核失败！具体原因为：'.$wxapp['fail_reason'].'。请优化后重新提交！', '', 'success');
		...
		if(!$result['status']){
			pdo_update('account_wxapp',array('status'=>2,'fail_reason'=>''),array('uniacid'=>$uniacid));
			itoast('审核成功！请点击一键发布按钮发布！','', 'success');
		}elseif($result['status']==1){
			pdo_update('account_wxapp',array('status'=>3,'fail_reason'=>$result['Reason']),array('uniacid'=>$uniacid));
			itoast('审核失败！具体原因为：'.$result['Reason'].'。请优化后重新提交！', url('account/manage'), 'success');
		}elseif($result['status']==2){
			itoast('该小程序还在审核中！请耐心等待！'.$res['content'],'', 'success');
		}else{
			itoast('未知错误详情：'.$res['content'],'', 'success');
		}
	}
	public function check_access_token(&$wxapp){
		if($wxapp['expirein']<TIMESTAMP){
			...
			pdo_update('account_wxapp',$update,array('uniacid'=>$wxapp['uniacid']));
			$wxapp['access_token']=$update['access_token'];
		}
	}
	//发布
	public function release($uniacid){
		$wxapp=pdo_get('account_wxapp',array('uniacid'=>$uniacid));
		if(!$wxapp['status']){
			itoast('请先上传审核小程序！', '', '');
		...
		pdo_update('account_wxapp',array('status'=>4,'fail_reason'=>''),array('uniacid'=>$uniacid));
		return true;
	}
}