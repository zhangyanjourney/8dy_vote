...
function file_cert($file, $type = 'cert_file', $name = '') {
	$harmtype = array('asp', 'php', 'jsp', 'js', 'css', 'php3', 'php4', 'php5', 'ashx', 'aspx', 'exe', 'cgi');
	$type = array('pem');
	if (empty($file)) {
		return error(-1, '没有上传内容');
	}
	//if (!in_array($type, array('cert_file', 'key_file', 'root_file'))) {
	//	return error(-2, '未知的上传类型');
	//}
	global $_W;
	$ext = pathinfo($file['name'], PATHINFO_EXTENSION);
	$ext = strtolower($ext);
	if (!in_array(strtolower($ext), $type) || in_array(strtolower($ext), $harmtype)) {
		return error(-3, '不允许上传此类文件');
	}
	$result = array();
	mkdirs(dirname(ATTACHMENT_ROOT . '/' . $name));
