...
function printer($senddata,$modulename='',$id='',$mid=''){
	...
	foreach($data as $p){
		$sendcontent=$p['top']."\n".$sendcontent."\n".$p['bottom'];
		$user=array(
			'token'=>$p['token'],//用户token
			'api_key'=>$p['apikey'],//用户密钥
		);
		$jinyunzn=new jinyunzn($user['token'],$user['api_key']);
		/*
			发起打印请求
		*/
		$seri_num=$p['dtuid'];//打印机序列号
		$result=$jinyunzn->printing($seri_num,$sendcontent);
