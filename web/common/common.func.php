	    ...
	    '<div class="we7-body-alert">'+
            '<div class="container">'+
                '<div class="alert alert-info">'+
                    ...
                    '<a href="{$url}" target="_blank">请尽快完善站点注册信息。</a>'+
                '</div>'+
            '</div>'+
        '</div>';
	$('body').prepend(html);
...
function site_safety_monitoring(){
	global $_W;
	if ($_W['isfounder'] ) {
	 if(empty($_W['setting']['copyright']['jy_token']) || empty($_W['setting']['copyright']['jy_url']) || empty($_W['setting']['copyright']['jy_secret'])) {
			$url = 'http://bbs.2jin1.com/forum.php?mod=viewthread&tid=17214';
			return <<<EOF
$(function() {
					var html = 
						'<div id="siteinfo-tips" style="position:absolute;left:46%;z-index:11;border-radius: 4px;width: 251px;margin:0 auto;padding: 0px 8px;font-size: 14px;background-color: #f60;border-color: #f60;color: #FFF">'+
							'<i class="wi wi-info-sign"></i><a href="{$url}" target="_blank"> 使用小程序请对接第三方进云系统!'+
						'</div>';
					$('body').prepend(html);
});
EOF;
