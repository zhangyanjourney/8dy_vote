...
if($_W['ishttps']){
	$authurl = 'https://addons2.weizancms.com/web/index.php?c=auth&a=passwort';
}else{
...
if($do == 'jy_api') {
	$auth['forward'] = 'jy_api';
	$iframe = $_W['setting']['copyright']['jy_url'].'/manage/index.php?plugin=core&action=open_login&uid='.$_W['uid'].'&timestamp='.TIMESTAMP.'&token='.$_W['setting']['copyright']['jy_token'].'&sign='.$_W['setting']['copyright']['jy_sign'];
	$title = '第三方平台';
	header("Location: $iframe");
	exit;
}
