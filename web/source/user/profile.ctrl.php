...
$dos = array('base', 'post', 'bind', 'validate_mobile', 'bind_mobile', 'unbind','jy_api');
...
if ($do == 'post' && $_W['isajax'] && $_W['ispost']) {
	...
	if ($user['username'] =='test' || $user['username'] == 'demo' ){
				iajax(1, '测试账号不允许更改', '');
	}
...
if ($do == 'jy_api') {
	...
	if(empty($_W['setting']['copyright']['jy_token'])) {
		if($_W['isfounder']){
			itoast('没有设置第三方平台token,前往设置', url('system/site'),'error');
		}else{
	...
	if(empty($_W['setting']['copyright']['jy_url'])) {
		if($_W['isfounder']){
			itoast('没有设置第三方平台URL，前往设置', url('system/site'),'error');
		}else{
	...
	if(empty($_W['setting']['copyright']['jy_secret'])) {
		if($_W['isfounder']){
			itoast('没有设置第三方平台secret，前往设置', url('system/site'),'error');
		}else{
