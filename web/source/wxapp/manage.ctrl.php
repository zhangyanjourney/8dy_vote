...
if($do == 'sh') {
	...
	if(!$wxapp['access_token']){
		...
		$url=$account_platform->getAuthLoginUrl(2,$uniacid);
		if(strexists($url,'javascript:alert')){
			itoast('缺少接入平台关键数据，等待微信开放平台推送数据，请十分钟后再试或是检查“授权事件接收URL”是否写错（index.php?c=account&a=auth&do=ticket地址中的&符号容易被替换成&amp;）'.$url, referer(), 'error');
		}else{
			header('Location:'.$url);
			exit;
		}
		...
		load()->classs('weixin.platform');
		$account_platform = new WeiXinPlatform();
		$account_platform->build($uniacid,$version);
	}
...
if($do == 'check'){
	...
	load()->classs('weixin.platform');
	$account_platform = new WeiXinPlatform();
	$account_platform->check($uniacid);
}
if($do == 'release') {
	//$appid=pdo_getcolumn('account_wxapp',array('uniacid'=>$uniacid),'key');
	load()->func('communication');
	load()->classs('weixin.platform');
	$account_platform = new WeiXinPlatform();
	$account_platform->release($uniacid);
